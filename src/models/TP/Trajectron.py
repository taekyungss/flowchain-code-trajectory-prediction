...
class Trajectron(torch.nn.Module):
    def __init__(self, cfg):
        ...
        with open("src/data/TP/Trajectron-plus-plus/experiments/pedestrians/models/eth_vel/config.json", 'r', encoding='utf-8') as conf_json:
        ...
        self.model_registrar = ModelRegistrar(cfg.OUTPUT_DIR, "cuda")
        ...
        for node_type in train_env.NodeType:
            if node_type not in hyperparams['pred_state']:
                continue
            self.optimizer[node_type] = optim.Adam([{'params': self.model_registrar.get_all_but_name_match('map_encoder').parameters()},
                                            {'params': self.model_registrar.get_name_match('map_encoder').parameters(), 'lr':0.0008}], lr=hyperparams['learning_rate'])
    ...
    def predict(self, data_dict, return_prob=False):
        ...
        predictions = predictions / 2  #HACK devide by std
        data_dict[("pred_st", 0)] = predictions[0]
        if return_prob:
            y_dist = model.dynamic.integrate_distribution(y_dist)
            data_dict[("prob", 0)] = y_dist
            y_dist.log_prob(torch.rand_like(predictions).tile(1, 10000, 1, 1))  # calculate the probability for fair time comparison
            
            estimate_gt_traj = True
            if estimate_gt_traj:
                data_dict["gt_traj_log_prob"] = y_dist.log_prob(data_dict["gt"])
        return data_dict           
    ...
    def update(self, data_dict):
        ...
        train_loss = self.train_loss(batch, "PEDESTRIAN")
        
        self.optimizer["PEDESTRIAN"].zero_grad()
        train_loss.backward()
        self.optimizer["PEDESTRIAN"].step()
    ...
    def load(self, path: Path=None) -> int:
        ...
        try:
            ...
            optimizer_to_cuda(self.optimizer["PEDESTRIAN"])
            epoch = ckpt["epoch"]
        except KeyError:
            epoch = 0
...
class ModelRegistrar(torch.nn.Module):
